...
class _IndexedListViewState extends State<IndexedListView> {
  ...
  Widget _getAlphabetItem(String alphabet) {
    return Expanded(
      child: Text(
        ...
        style: TextStyle(fontSize: 20.0, fontFamily: 'Pochaevsk'),
      ),
    );
  }
  ...
  Map<String, int> _getAlphabetDyPositions(List<Prodiom> items) {
    Map<String, int> alphabetDyPositions = {};
    for (var i = 0; i < items.length; i++) {
      final firstChar = items[i].prodiom.toString()[0];
  ...
  Widget _alphabeticalIndex(BuildContext context, List<Prodiom> items) {
    List<String> alphabets = getAlphabetsFromStringList(
        items.map((item) => item.prodiom.toString()).toList());
